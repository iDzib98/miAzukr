...
export default function FoodBarChart() {
    ...
    useEffect(() => {
        ...
        ;(async () => {
            ...
            try {
                ...
                // indicate whether there are any records (seriesData may be populated with zeros)
                console.log('FoodBarChart - fetched records:', recs)
                // Only consider records of type 'Alimentación' as valid for this chart
                setHasRecs(Array.isArray(recs) && recs.some(r => r.type === 'Alimentación'))
    ...
    return (
        <Card elevation={3} sx={{ mb: 2 }}>
            <CardHeader
                title="Alimentación"
                subheader={`Carbohidratos por día`}
                action={(
